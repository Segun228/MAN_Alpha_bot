...
async def get_labels(message):
    ...
    user_message = {"role": "user",  # Юзер промпт, стоит по максимуму сократить
                    "content": f"{message}. Выбери не более двух самых важных тем строго из предложенных для решения этого вопроса"
                               f" и верни их через запятую. Ничего больше не возвращай. Свои темы придумывать строго запрещено"
...
async def get_docs_from_category(message, category):
    ...
    data = {
        ...
        "messages": [
            ...
            {
                "role": "user",
                "content": f"Выбери не более 5 документов, в которые полнзны для ответа на вопрос: {message} и верни только их"
                           f"через запятую. Если не нашёл подходящего, верни строго пустую строку и ничего больше в формате '' "
                           f"Список документов: {documents}. ."
            }
        ],
        "temperature": 0.5  # Можно поставить выше, пока золотая середина
    }
    try:
