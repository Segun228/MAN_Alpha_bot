...
from .recomendations import check_prompt, check_http_prompt
...
@app.post("/defend/http")
async def defend_prompt_http(request: Request):
    try:
        data = await request.json()
        telegram_id = data.get("telegram_id", "undefined")
        prompt = data.get("prompt")
        ...
        result = await check_http_prompt( 
            prompt=prompt,
            words_count=words_count
        )
        logging.info(f"Request {prompt} is {'safe' if result.get('is_safe') else 'NOT safe'}")
...
@app.post("/defend/multiple/http")
async def defend_prompt_multiple_http(request: Request):
    try:
        data = await request.json()
        telegram_id = data.get("telegram_id", "undefined")
        prompt = data.get("prompt")
