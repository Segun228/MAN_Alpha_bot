...
ссылка на дашборд и гейтвей (login = admin, password = admin)
http://62.113.44.224:3000/ 
...
1. Перейти по ссылке [http://localhost:3000](http://localhost:3000) (или по указанному в `.env` порту, либо же на 3000 порт удаленного ссервера, адрес которого будет прикреплен с решением).
...
другие виды взаимодействия, например на внутренние продукты банка. Также были созданы кейсы для Smoke-тестирования. В 
дальнейшем базу тестов можно в кратчайшие сроки дополнить и развить в соотвествии с появляющимся функционалом.
...
- **Интеграции:** 
  ...
  - Kafka для асинхронных событий (регистрация, изменения профиля)
